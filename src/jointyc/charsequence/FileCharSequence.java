 ...
 * support long indexing, so that the maximum valid file size is equal to
...
public class FileCharSequence implements CharSequence {
	/**
	 * Default page size: {@value #DEFAULT_PAGE_SIZE} bytes
	 */
	public static final int DEFAULT_PAGE_SIZE = 1 * 1024 * 1024; // 1 MB for single page

	/**
	 * Default pages count: {@value #DEFAULT_PAGES_COUNT} pages
	 */
	public static final int DEFAULT_PAGES_COUNT = 200; // 200 MB in total

	/**
	 * Default unicode flag: {@value #DEFAULT_IS_UNICODE}
	 */
	...
	public char charAt(int index) {
		...
		Integer pageStart = Integer.valueOf(cache.pageSize * (int) (index / cache.pageSize));
		byte[] page;
		if (cache.containsKey(pageStart)) {
			page = cache.get(pageStart);
		} else {
